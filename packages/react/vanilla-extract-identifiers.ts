 ...
 * `sandbar-` prefix required by CLAUDE.md — vanilla-extract does not prefix
 ...
 */
export function sandbarUiIdentifiers({ hash, debugId }: { hash: string; debugId?: string }): string {
  const readable = (debugId ?? hash).replace(/[^a-zA-Z0-9]+/g, '-').toLowerCase();
  return `sandbar-${readable}`;
}
