...
| Framework (docs only) | `next`, `react`, `react-dom` | React 19; `@sandbar-ui/react` itself supports React 18/19 per the CI matrix |
...
| Icons build | SVGO + a codegen script | `@sandbar-ui/icons` source SVGs are generated into tree-shakeable components, not hand-written per icon |
| Lint tooling (for `@sandbar-ui/eslint-plugin` authoring) | `@typescript-eslint/utils`, `eslint` | |
...
| Radix Primitives, MUI, Chakra, or any other component library as a runtime dependency of `@sandbar-ui/react` | Sandbar UI builds its own headless primitives in `@sandbar-ui/core` — depending on a competing library undermines bundle-size and API-consistency goals | Hand-built primitives in `@sandbar-ui/core`, informed by the WAI-ARIA APG, not wrapped |
...
- **Forbidden**: GPL, AGPL, SSPL, proprietary/commercial without approval — a copyleft dependency in `@sandbar-ui/react` or `@sandbar-ui/core` propagates an obligation to every downstream consumer, which is unacceptable for a published library
...
5. Get explicit approval in the plan before merging — **do not install mid-task without approval**, and never as a runtime dependency of `@sandbar-ui/react`/`@sandbar-ui/core` without discussing the bundle-size tradeoff explicitly
