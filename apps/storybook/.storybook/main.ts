...
import { sandbarUiIdentifiers } from '../../../packages/react/vanilla-extract-identifiers';
...
const config: StorybookConfig = {
  ...
  // specific to one component). `sandbarUiIdentifiers` matches the same
  ...
  viteFinal: async (viteConfig) => {
    viteConfig.plugins ??= [];
    viteConfig.plugins.push(vanillaExtractPlugin({ identifiers: sandbarUiIdentifiers }));
    return viteConfig;
  },
};
