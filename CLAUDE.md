...
Every component ships with: implementation (full TS types, no `any`), a recipe/style definition with all variants, unit + interaction tests, an axe a11y test, a Storybook visual regression story, an MDX docs page (anatomy diagram, live example, variant gallery, auto-generated props table, keyboard table, data-attributes table, a11y notes, composition recipes, styling section, source links), and a changeset entry. Incomplete = not done.
